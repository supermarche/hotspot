...
class Sentinel2Downloader:
    ...
    def create_evalscript(self):
        ...
        bands_formula = ', '.join([f'1*sample.{band}' for band in self.bands])
    ...
    def create_payload_template(self):
        ...
        return {
            "input": {
                "bounds": {
                    "properties": {"crs": "http://www.opengis.net/def/crs/EPSG/0/25833"},
                    "bbox": self.bbox
                },
                "data": [{
...
if __name__ == "__main__":
    ...
    bbox = [497839, 5665784, 500190, 5668313]
    ...
    for date in intersecting_dates:
        ...
        #data.save_data(f"../data/Sentinel-2/{date}_{downloader.satellite_type}.tiff")
        with open(f"../data/Sentinel-2/{date}_{downloader.satellite_type}.tiff", 'wb') as f:
            f.write(data)
        import pdb
        pdb.set_trace()
